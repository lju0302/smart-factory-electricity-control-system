...
def insert_predictions_to_db(predictions_list):
    """
    score.py 결과 중 기존 핵심 8개 컬럼 + 추가 상세 컬럼을 anomaly_predictions 테이블에 INSERT합니다.
    """
        ...
        for pred in predictions_list:
            records_to_insert.append(
                (
                    ...
                    parse_created_at(pred.get("created_at")),
...
def event_hub_trigger(azeventhub: func.EventHubEvent):
    try:
        ...
        alert_summary = send_alerts_to_teams(prediction_rows)

        logging.info(
            "Teams alert summary. sent=%s, skipped=%s, failed=%s",
            alert_summary["sent_count"],
            alert_summary["skipped_count"],
            alert_summary["failed_count"],
        )
