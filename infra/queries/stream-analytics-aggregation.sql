-- Stream Analytics aggregation query
-- Scope: real-time 1-minute and 15-minute outputs only.
-- 1-hour and 1-day outputs are created by SQL stored procedures invoked by
-- the sql-timer-trigger Function App.
...
WITH PrepPowerInput AS (
    SELECT
        ...
        CASE
            WHEN CAST(voltageR AS float) >= CAST(voltageS AS float)
             AND CAST(voltageR AS float) >= CAST(voltageT AS float)
                THEN CAST(voltageR AS float)
            WHEN CAST(voltageS AS float) >= CAST(voltageR AS float)
             AND CAST(voltageS AS float) >= CAST(voltageT AS float)
                THEN CAST(voltageS AS float)
            ELSE CAST(voltageT AS float)
        END AS v_max,
        CASE
            WHEN CAST(currentR AS float) >= CAST(currentS AS float)
             AND CAST(currentR AS float) >= CAST(currentT AS float)
                THEN CAST(currentR AS float)
            WHEN CAST(currentS AS float) >= CAST(currentR AS float)
             AND CAST(currentS AS float) >= CAST(currentT AS float)
                THEN CAST(currentS AS float)
    ...
    FROM [powerinput]
    TIMESTAMP BY localtime_datetime
),
...
Aggregated1m AS (
    SELECT
        ...
        CASE
            WHEN MIN(CAST(voltageRS AS float)) <= 360.0
              ...
              OR MIN(CAST(voltageTR AS float)) <= 360.0
                THEN 1
            WHEN (
                AVG(CAST(powerFactorR AS float))
                + AVG(CAST(powerFactorS AS float))
                + AVG(CAST(powerFactorT AS float))
            ) / 3.0 <= 90.0
                THEN 1
            ELSE 0
        END AS is_anomaly,
        ...
        AVG((
            CAST(powerFactorR AS float)
            + CAST(powerFactorS AS float)
            + CAST(powerFactorT AS float)
        ) / 3.0) AS avg_power_factor,
        AVG((
            CASE
                WHEN CAST(voltageR AS float) >= CAST(voltageS AS float)
                 AND CAST(voltageR AS float) >= CAST(voltageT AS float)
            ...
            END
            - (
                CAST(voltageR AS float)
                + CAST(voltageS AS float)
                + CAST(voltageT AS float)
            ) / 3.0
        ) / NULLIF((
            CAST(voltageR AS float)
            + CAST(voltageS AS float)
            + CAST(voltageT AS float)
        ) / 3.0, 0) * 100.0) AS avg_voltage_imbalance_rate,
        AVG((
            CASE
                WHEN CAST(currentR AS float) >= CAST(currentS AS float)
                 AND CAST(currentR AS float) >= CAST(currentT AS float)
            ...
            END
            - (
                CAST(currentR AS float)
                + CAST(currentS AS float)
                + CAST(currentT AS float)
            ) / 3.0
        ) / NULLIF((
            CAST(currentR AS float)
            + CAST(currentS AS float)
            + CAST(currentT AS float)
        ) / 3.0, 0) * 100.0) AS avg_current_imbalance_rate,
...
ModelInput AS (
    SELECT
        ...
        CASE
            WHEN DATEPART(weekday, window_end_time) IN (1, 7) THEN 1
            ELSE 0
        END AS is_weekend
    FROM Aggregated15m
)
